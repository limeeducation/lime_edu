<?php
include($_SERVER['DOCUMENT_ROOT'].'/application/views/layout/head.php');
//include_once($_SERVER['DOCUMENT_ROOT']."/inc/blog_list.php");
?>
<link rel="stylesheet" type="text/css" href="css/sub.css">
<body>
    <script type="text/javascript">
        function change_nation(tab,nation){
            if(tab == "5"){

            }else if(tab == "6"){
                $(".banr_lists").hide();
                $("#banr_list_"+nation).show();
            }else if(tab == "7"){
				$(".natio_btn_area").hide();
				$("#btn_"+nation+"_area").show();
				$(".banr_lists").hide();
				$("#banr_list_"+nation).show();
            }
        }

        $('.natio_btn_area').each(function() {

			var nowImg = $(this).find('img');  //호버한 부분의 img파일 찾기
			var srcName = nowImg.attr('src');  //호버한 부분의 이미지 주소값 src가지고오기
			var newSrc = srcName.substring(0, srcName.lastIndexOf('.'));
			//.png , .jpg 와같이 파일명 자르기. 뒤에서부터 시작해서 '.'점있는 곳 까지 컷!

			//호버이벤트
			$(this).hover(function() {
				$(this).find('img').attr('src', newSrc+ '_after.' + /[^.]+$/.exec(srcName)); //hover시 _on붙이기
			}, function() {
				$(this).find('img').attr('src', newSrc + '.' + /[^.]+$/.exec(srcName)); //hover시 _on 때기
			});
        });
    </script>
	<div id="wrap">
	<?php
    include($_SERVER['DOCUMENT_ROOT'].'/application/views/layout/header.php');
    ?>
		<div id="container">

			<div id="content">

			<!-- cont_sect -->
				<section class="cont_sect sub2_1">
					<div class="inner">
						<div class="cont_title">미국 조기유학 프로그램</div>
						<ul class="ico_btn_list type3">
							<li>
								<a href="#">
									<i class="ico ico1"></i>
									<div class="text">조기유학 학교찾기</div>
								</a>
							</li>
							<li>
								<a href="#">
									<i class="ico ico2"></i>
									<div class="text">왜 조기유학인가?</div>
								</a>
							</li>
							<li>
								<a href="#">
									<i class="ico ico3"></i>
									<div class="text">명문 TOP50</div>
								</a>
							</li>
						</ul>
					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_2">
					<div class="inner">
						<div class="cont_image"><img src="/static/img/early_abroad/usa/sub2_2_img1@2x.png" alt=""></div>
					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_3">
					<div class="inner">
						<div class="national_flag"><img src="/static/img/early_abroad/usa/national_flag_eng@2x.png" alt="미국"></div>
						<div class="cont_title"><div class="box">미국 교육제도</div></div>
						<div class="cont_box line">
							<div>
								<p>미국의 교육제도는 <b>초등학교, 중·고등학교 과정</b>이 우리 나라에 비해 <b>다양한 점</b>이 특징입니다.</p>
								<p><b>공립학교의 교육</b>에 관해서는 <b>주정부가 관여하는 부분과</b> 각 <b>지역교육청이 독립적으로 관장하는  부분</b>으로 나뉘어집니다.</p>
								<p>각 지역교육청에는 의사결정기관 또는 자문기관으로서 교육위원회(District Board of Education)가 있어서 <br>교육과정 등 각종 공립학교정책 수립, 휴무일 지정 등을 의결 또는 자문하며, <br>교육감은 교장·교원과 더불어 각 학교의 운영 및 교육을 실시합니다.</p>
							</div>
							<div>
								<p>교육비는 연방정부, 주정부, 시(市) 또는 County가 부담하는데,연방정부는 약 7~8%정도밖에 부담하지 않습니다.</p>
								<p>K-12로 표시되는 유·초·중등과정은 일반적으로 13년제로 초등학교 6~8년(유치원 1년 별도), <br>중학교 2~3년, 고등학교 3~4년으로 나누어지며, 유치원과정의 의무교육여부는 주에 따라 다릅니다.</p>
							</div>
						</div>

						<ul class="num_list">
							<li>
								<div>
									<span class="num">01</span>
									<span class="text">유아 프로그램(Nursery to Pre-K) — 2~5세</span>
								</div>
							</li>
							<li>
								<div>
									<span class="num">02</span>
									<span class="text">초등학교(1-5학년) — 6-10세</span>
								</div>
							</li>
							<li>
								<div>
									<span class="num">03</span>
									<span class="text">중학교 또는 중학교(6-8학년) — 11-14세</span>
								</div>
							</li>
							<li>
								<div>
									<span class="num">04</span>
									<span class="text">고등학교(9-12학년) — 15-18세</span>
								</div>
							</li>
						</ul>
					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_4">
					<div class="inner">

						<div class="cont_title"><div class="box">보딩스쿨 이란?</div></div>
						<div class="cont_box">
							<div>
								<p><b>기숙사가 있는 학교</b>를 <b>보딩스쿨 Boarding School</b>이라고 하며, <br>보통 기숙사는 학교 내에 있거나 혹은 학교 인근에 있는 경우가 많습니다.</p>
								<p>미국에서 명문 사립학교 중 기숙사형태로 운영되는 학교가 많은 이유는 <br>학생들이 다른 나라들에서 유학 온 학생들은 물론, 미국 내 다른 주에서 온 학생들도 많기 때문입니다.</p>
							</div>
						</div>
						<div class="cont_image"><img src="/static/img/early_abroad/usa/sub2_4_img1@2x.png" alt=""></div>

						<div class="cont_title ranking_title">
							탑보딩스쿨 Niche Ranking 50위
							<a href="#" class="ranking_more">전체보기 +</a>
						</div>
						<table class="cont_table ranking_table">
							<colgroup>
								<col>
								<col style="width:34%">
								<col style="width:15%">
								<col>
								<col>
								<col>
								<col>
								<col>
								<col>
							</colgroup>
							<thead>
								<tr>
									<th>랭킹</th>
									<th>학교이름</th>
									<th>지역</th>
									<th>학년</th>
									<th>학생수</th>
									<th>SAT 평균</th>
									<th>ACT 평균</th>
									<th>AP 과목수</th>
									<th>학비</th>
								</tr>
							</thead>
							<tbody>
								<tr class="top20">
									<td>1</td>
									<td>Phillips Academy Andover</td>
									<td>Andover, MA•</td>
									<td>9-12</td>
									<td>1,187</td>
									<td>1470</td>
									<td>33</td>
									<td>N/A</td>
									<td>66290</td>
								</tr>
								<tr class="top20">
									<td>2</td>
									<td>Choate Rosemary Hall</td>
									<td>Wallingford, CT•</td>
									<td>9-12</td>
									<td>868</td>
									<td>1428</td>
									<td>33</td>
									<td>N/A</td>
									<td>65,820</td>
								</tr>
								<tr class="top20">
									<td>3</td>
									<td>Phillips Exeter Academy</td>
									<td>Exeter, NH•</td>
									<td>9-12</td>
									<td>1,077</td>
									<td>1450</td>
									<td>33</td>
									<td>N/A</td>
									<td>61,121</td>
								</tr>
								<tr class="top20">
									<td>4</td>
									<td>Groton School</td>
									<td>Groton, MA•</td>
									<td>8-12</td>
									<td>380</td>
									<td>1420</td>
									<td>31</td>
									<td>8</td>
									<td>59,995</td>
								</tr>
								<tr class="top20">
									<td>5</td>
									<td>The Hotchkiss School</td>
									<td>Lakeville, CT•</td>
									<td>9-12</td>
									<td>599</td>
									<td>1420</td>
									<td>32</td>
									<td>17</td>
									<td>65,490</td>
								</tr>
								<tr class="top20">
									<td>6</td>
									<td>Princeton International School of Math and Science</td>
									<td>Princeton, NJ•</td>
									<td>9-12</td>
									<td>120</td>
									<td>1490</td>
									<td>31</td>
									<td>17</td>
									<td>61,500</td>
								</tr>
								<tr class="top20">
									<td>7</td>
									<td>St. Paul's School</td>
									<td>Concord, NH•</td>
									<td>9-12</td>
									<td>530</td>
									<td>1410</td>
									<td>32</td>
									<td>N/A</td>
									<td>62,000</td>
								</tr>
								<tr class="top20">
									<td>8</td>
									<td>The Lawrenceville School</td>
									<td>Lawrenceville, NJ•</td>
									<td>9-12</td>
									<td>822</td>
									<td>1440</td>
									<td>32</td>
									<td>N/A</td>
									<td>73,220</td>
								</tr>
								<tr class="top20">
									<td>9</td>
									<td>Deerfield Academy</td>
									<td>Deerfield, MA•</td>
									<td>9-12</td>
									<td>658</td>
									<td>1420</td>
									<td>32</td>
									<td>9</td>
									<td>61,840</td>
								</tr>
								<tr class="top20">
									<td>10</td>
									<td>Noble and Greenough School</td>
									<td>Dedham, MA•</td>
									<td>7-12</td>
									<td>614</td>
									<td>1410</td>
									<td>33</td>
									<td>13</td>
									<td>60,300</td>
								</tr>
								<tr class="top20">
									<td>11</td>
									<td>The Loomis Chaffee School</td>
									<td>Windsor, CT•</td>
									<td>9-12</td>
									<td>741</td>
									<td>1400</td>
									<td>32</td>
									<td>N/A</td>
									<td>65,350</td>
								</tr>
								<tr class="top20">
									<td>12</td>
									<td>Concord Academy</td>
									<td>Concord, MA•</td>
									<td>9-12</td>
									<td>398</td>
									<td>1390</td>
									<td>32</td>
									<td>18</td>
									<td>65,850</td>
								</tr>
								<tr class="top20">
									<td>13</td>
									<td>Milton Academy</td>
									<td>Milton, MA•</td>
									<td>K-12</td>
									<td>1,020</td>
									<td>1400</td>
									<td>32</td>
									<td>N/A</td>
									<td>65,950</td>
								</tr>
								<tr class="top20">
									<td>14</td>
									<td>Cate School</td>
									<td>Carpinteria, CA•</td>
									<td>9-12</td>
									<td>298</td>
									<td>1410</td>
									<td>32</td>
									<td>19</td>
									<td>65,500</td>
								</tr>
								<tr class="top20">
									<td>15</td>
									<td>Middlesex School</td>
									<td>Concord, MA•</td>
									<td>9-12</td>
									<td>409</td>
									<td>1420</td>
									<td>32</td>
									<td>20</td>
									<td>69,280</td>
								</tr>
								<tr class="top20">
									<td>16</td>
									<td>The Thacher School</td>
									<td>Ojai, CA•</td>
									<td>9-12</td>
									<td>259</td>
									<td>1420</td>
									<td>32</td>
									<td>N/A</td>
									<td>67,940</td>
								</tr>
								<tr class="top20">
									<td>17</td>
									<td>St. Mark's School</td>
									<td>Southborough, MA•</td>
									<td>9-12</td>
									<td>385</td>
									<td>1390</td>
									<td>31</td>
									<td>N/A</td>
									<td>69,960</td>
								</tr>
								<tr class="top20">
									<td>18</td>
									<td>Lake Forest Academy</td>
									<td>Lake Forest, IL•</td>
									<td>9-12</td>
									<td>437</td>
									<td>1390</td>
									<td>30</td>
									<td>24</td>
									<td>61,500</td>
								</tr>
								<tr class="top20">
									<td>19</td>
									<td>Cranbrook Schools</td>
									<td>Bloomfield Hills, MI•</td>
									<td>PK, K-12</td>
									<td>1,659</td>
									<td>1400</td>
									<td>31</td>
									<td>15</td>
									<td>52,350</td>
								</tr>
								<tr class="top20">
									<td>20</td>
									<td>American Heritage Schools, Broward Campus</td>
									<td>Plantation, FL•</td>
									<td>PK, K-12</td>
									<td>2,794</td>
									<td>1380</td>
									<td>31</td>
									<td>24</td>
									<td>60,000</td>
								</tr>
								<tr>
									<td>21</td>
									<td>The Peddie School</td>
									<td>Hightstown, NJ•</td>
									<td>9-12</td>
									<td>532</td>
									<td>1390</td>
									<td>32</td>
									<td>22</td>
									<td>67,900</td>
								</tr>
								<tr>
									<td>22</td>
									<td>The Madeira School</td>
									<td>Mclean, VA•</td>
									<td>9-12</td>
									<td>343</td>
									<td>1360</td>
									<td>31</td>
									<td>18</td>
									<td>63,250</td>
								</tr>
								<tr>
									<td>23</td>
									<td>St. Albans School</td>
									<td>Washington, DC•</td>
									<td>4-12</td>
									<td>592</td>
									<td>1410</td>
									<td>33</td>
									<td>13</td>
									<td>74,120</td>
								</tr>
								<tr>
									<td>24</td>
									<td>Belmont Hill School</td>
									<td>Belmont, MA•</td>
									<td>7-12</td>
									<td>465</td>
									<td>1420</td>
									<td>33</td>
									<td>N/A</td>
									<td>65,640</td>
								</tr>
								<tr>
									<td>25</td>
									<td>American Heritage Schools, Palm Beach Campus</td>
									<td>Delray Beach, FL•</td>
									<td>PK, K-12</td>
									<td>1,797</td>
									<td>1360</td>
									<td>30</td>
									<td>24</td>
									<td>60,000</td>
								</tr>
								<tr>
									<td>26</td>
									<td>The Webb Schools</td>
									<td>Claremont, CA•</td>
									<td>9-12</td>
									<td>404</td>
									<td>1380</td>
									<td>31</td>
									<td>21</td>
									<td>73,320</td>
								</tr>
								<tr>
									<td>27</td>
									<td>St. Andrew's School</td>
									<td>Middletown, DE•</td>
									<td>9-12</td>
									<td>310</td>
									<td>1370</td>
									<td>32</td>
									<td>N/A</td>
									<td>57,000</td>
								</tr>
								<tr>
									<td>28</td>
									<td>Thomas Jefferson School</td>
									<td>Saint Louis, MO•</td>
									<td>7-12</td>
									<td>85</td>
									<td>1420</td>
									<td>32</td>
									<td>14</td>
									<td>62,600</td>
								</tr>
								<tr>
									<td>29</td>
									<td>St. Stephen's Episcopal School</td>
									<td>Austin, TX•</td>
									<td>6-12</td>
									<td>699</td>
									<td>1380</td>
									<td>32</td>
									<td>20</td>
									<td>72,570</td>
								</tr>
								<tr>
									<td>30</td>
									<td>Hackley School</td>
									<td>Tarrytown, NY•</td>
									<td>K-12</td>
									<td>855</td>
									<td>1410</td>
									<td>32</td>
									<td>13</td>
									<td>67,475</td>
								</tr>
								<tr>
									<td>31</td>
									<td>Kent School</td>
									<td>Kent, CT•</td>
									<td>9-12</td>
									<td>520</td>
									<td>1370</td>
									<td>31</td>
									<td>N/A</td>
									<td>69,950</td>
								</tr>
								<tr>
									<td>32</td>
									<td>Mercersburg Academy</td>
									<td>Mercersburg, PA•</td>
									<td>9-12</td>
									<td>447</td>
									<td>1330</td>
									<td>30</td>
									<td>32</td>
									<td>66,900</td>
								</tr>
								<tr>
									<td>33</td>
									<td>The Taft School</td>
									<td>Watertown, CT•</td>
									<td>9-12</td>
									<td>609</td>
									<td>1370</td>
									<td>31</td>
									<td>29</td>
									<td>66,900</td>
								</tr>
								<tr>
									<td>34</td>
									<td>Episcopal High School</td>
									<td>Alexandria, VA•</td>
									<td>9-12</td>
									<td>464</td>
									<td>1350</td>
									<td>30</td>
									<td>22</td>
									<td>66,360</td>
								</tr>
								<tr>
									<td>35</td>
									<td>Blair Academy</td>
									<td>Blairstown, NJ•</td>
									<td>9-12</td>
									<td>465</td>
									<td>1340</td>
									<td>31</td>
									<td>22</td>
									<td>70,200</td>
								</tr>
								<tr>
									<td>36</td>
									<td>'Iolani School</td>
									<td>Honolulu, HI•</td>
									<td>K-12</td>
									<td>2,181</td>
									<td>1370</td>
									<td>31</td>
									<td>30</td>
									<td>61,200</td>
								</tr>
								<tr>
									<td>37</td>
									<td>The Athenian School</td>
									<td>Danville, CA•</td>
									<td>6-12</td>
									<td>536</td>
									<td>1380</td>
									<td>32</td>
									<td>N/A</td>
									<td>80,260</td>
								</tr>
								<tr>
									<td>38</td>
									<td>Northfield Mount Hermon</td>
									<td>Mount Hermon, MA•</td>
									<td>9-12</td>
									<td>655</td>
									<td>1350</td>
									<td>31</td>
									<td>N/A</td>
									<td>66,810</td>
								</tr>
								<tr>
									<td>39</td>
									<td>Miss Porter's School</td>
									<td>Farmington, CT•</td>
									<td>9-12</td>
									<td>334</td>
									<td>1320</td>
									<td>30</td>
									<td>N/A</td>
									<td>68,725</td>
								</tr>
								<tr>
									<td>40</td>
									<td>The Village School</td>
									<td>Houston, TX•</td>
									<td>PK, K-12</td>
									<td>1,773</td>
									<td>1350</td>
									<td>30</td>
									<td>21</td>
									<td>69,550</td>
								</tr>
								<tr>
									<td>41</td>
									<td>Sierra Canyon School</td>
									<td>Chatsworth, CA•</td>
									<td>PK, K-12</td>
									<td>1,154</td>
									<td>1330</td>
									<td>30</td>
									<td>25</td>
									<td>40,770</td>
								</tr>
								<tr>
									<td>42</td>
									<td>Western Reserve Academy</td>
									<td>Hudson, OH•</td>
									<td>9-12</td>
									<td>431</td>
									<td>1380</td>
									<td>30</td>
									<td>20</td>
									<td>68,425</td>
								</tr>
								<tr>
									<td>43</td>
									<td>Fairmont Preparatory Academy</td>
									<td>Anaheim, CA•</td>
									<td>9-12</td>
									<td>456</td>
									<td>1350</td>
									<td>31</td>
									<td>27</td>
									<td>67,120</td>
								</tr>
								<tr>
									<td>44</td>
									<td>United World College USA</td>
									<td>Montezuma, NM•</td>
									<td>11-12</td>
									<td>235</td>
									<td>1370</td>
									<td>31</td>
									<td>N/A</td>
									<td>36,750</td>
								</tr>
								<tr>
									<td>45</td>
									<td>The Hill School</td>
									<td>Pottstown, PA•</td>
									<td>9-12</td>
									<td>529</td>
									<td>1350</td>
									<td>31</td>
									<td>28</td>
									<td>61,410</td>
								</tr>
								<tr>
									<td>46</td>
									<td>Georgetown Preparatory School</td>
									<td>North Bethesda, MD•</td>
									<td>9-12</td>
									<td>496</td>
									<td>1350</td>
									<td>31</td>
									<td>24</td>
									<td>66,200</td>
								</tr>
								<tr>
									<td>47</td>
									<td>Indian Springs School</td>
									<td>Indian Springs, AL•</td>
									<td>8-12</td>
									<td>327</td>
									<td>1400</td>
									<td>31</td>
									<td>18</td>
									<td>56,000</td>
								</tr>
								<tr>
									<td>48</td>
									<td>Pomfret School</td>
									<td>Pomfret, CT•</td>
									<td>9-12</td>
									<td>350</td>
									<td>1320</td>
									<td>30</td>
									<td>N/A</td>
									<td>66,900</td>
								</tr>
								<tr>
									<td>49</td>
									<td>Suffield Academy</td>
									<td>Suffield, CT•</td>
									<td>9-12</td>
									<td>410</td>
									<td>1280</td>
									<td>29</td>
									<td>12</td>
									<td>66,500</td>
								</tr>
								<tr>
									<td>50</td>
									<td>The Masters School</td>
									<td>Dobbs Ferry, NY•</td>
									<td>5-12</td>
									<td>694</td>
									<td>1360</td>
									<td>31</td>
									<td>15</td>
									<td>63,000</td>
								</tr>
							</tbody>
						</table>

					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_5">
					<div class="inner">

						<div class="cont_title"><div class="box">관리형 유학이란?</div></div>
						<div class="cont_box line">
							<div>
								<p>아이 홀로 떠난 유학에서 <b>낯선 외국 생활에  문제없이 적응할 수 있도록 혹은 명문대학교 진학을 목표로 하는 학생들을 위한</b> <br><b>토탈케어 종합 프로그램</b>입니다. ESL 제공은 물론 성적관리를 위한 방과 후 수업 등의 학업적인 부분 뿐만 아니라 <br>건강, 음식,정서 등 다각도로 학생을 케어하고 써포트  합니다. 부모님을 대신하여 학교 카운셀러와 소통하고 주말과 휴일에도 <br>학교 밖에서 다채로운 운동, 문화체험 및 액티비티에 참석하게 되어 체력증진과 여러가지 경험을 통하여 성장하고 스스로 성장해가는 <br>자립심을 키우게 됩니다. 미국 명문 대학진학 및 전공선정에 있어 성공적인 포트폴리오를 만들수 있습니다. </p>
							</div>
							<div>
								<p>또한 부모님과 관리형 관리자들은 원활한 소통으로 자녀들의 일상 생활을 확인할 수 있으며 예측하지 못한 문제가 생길 경우에도 <br>적극적으로 해결하는 데 용이합니다. 언제든 어떤 위기 상황이 생기더라도 24시간 응대할 수 있는 관리자들로 인해 <br>우리 아이들과 부모님은 안심할 수 있는 유학을 계획 할 수 있습니다.</p>
							</div>
						</div>
						<div class="cont_image"><img src="/static/img/early_abroad/usa/sub2_5_img1@2x.png" alt=""></div>

						<div class="cont_title">관리형 프로그램 비교</div>
						<table class="cont_table program_table">
							<colgroup>
								<col>
								<col>
								<col>
								<col>
							</colgroup>
							<thead>
								<tr>
									<th>구분</th>
									<th>프리미엄 관리형</th>
									<th>스탠다드 관리형</th>
									<th>세미 관리형</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>숙소타입</td>
									<td>대학 캠퍼스 기숙사 <br>미국 주립대학교 기숙사 사용<br>남녀 기숙사 층 분리</td>
									<td>자체 외부기숙사<br>남녀 기숙사 건물 분리</td>
									<td>학교기숙사 or 프리미엄 외부기숙사</td>
								</tr>
								<tr>
									<td>방과 후 수업</td>
									<td>레벨별 ESL 수업<br>전과목 그룹 Tutoring<br>일대일 개인과외 Tutoring<br>SAT or TOEFL 수업<br>의대, 치대, 수의대,법대 준비반</td>
									<td>레벨별 ESL수업<br>숙제 및 자기 주도학습 Tutoring<br>SAT or TOEFL 수업</td>
									<td>ESL 수업<br>숙제 및 자기주도학습 Tutoring</td>
								</tr>
								<tr>
									<td>개인레슨 제공</td>
									<td>일대일 개인과외 제공</td>
									<td>제공안함</td>
									<td>제공안함</td>
								</tr>
								<tr>
									<td>식사</td>
									<td>대학 뷔페 및 한식 제공</td>
									<td>현지식/아시아 음식</td>
									<td>현지식/아시아 음식</td>
								</tr>
								<tr>
									<td>전용앱</td>
									<td>전용 App으로 학사일정 안내<br>학생 Daily일정 알람 및 체크<br>카카오톡 부모님 개별채팅</td>
									<td>없음</td>
									<td>전용 App으로 학사일정 안내<br>그룹 커뮤니케이션 채팅</td>
								</tr>
								<tr>
									<td>한국인 담당자</td>
									<td>한국인 관리자<br> 부모님과 카톡소통</td>
									<td>없음</td>
									<td>없음</td>
								</tr>
								<tr>
									<td>월별리포트</td>
									<td>매월 정기 레포트 제공</td>
									<td>매월 정기 레포트 제공</td>
									<td>매월 정기 레포트 제공</td>
								</tr>
								<tr>
									<td>통학방법</td>
									<td>스쿨버스, 자체차량</td>
									<td>스쿨버스, 자체차량</td>
									<td>스쿨버스, 자체차량</td>
								</tr>
								<tr>
									<td>외출 교통수단</td>
									<td>학생 개인일정 외출시<br>차량 제공(항시 전용기사 대기)</td>
									<td>없음</td>
									<td>없음</td>
								</tr>
								<tr>
									<td>주말 액티비티</td>
									<td>승마, 수영, 테니스 운동<br>매주 필드트립<br>지역행사, 종교활동<br>대학교 탐방<br>봉사활동</td>
									<td>매주 스페셜 액티비티<br>지역행사, 종교활동<br>봉사활동</td>
									<td>매주 엄선한 액티비티<br>봉사활동<br>지역행사, 종교할동<br>대학교 탐방<br>봄방학&추수감사절 단체트립</td>
								</tr>
							</tbody>
						</table>

						<div class="cont_title">관리형 프로그램 비교</div>
						<ul class="btn_list type3">
							<li><a href="#">프리미엄 관리형</a></li>
							<li><a href="#">스탠다드 관리형</a></li>
							<li><a href="#">세미 관리형</a></li>
						</ul>

					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_6">
					<div class="inner">

						<div class="cont_image"><img src="/static/img/early_abroad/usa/sub2_6_img1@2x.png" alt=""><div class="text">공립 교환학생 <br>프로그램</div></div>
						<div class="cont_title"><div class="box">미국 교환학생 프로그램 이란?</div></div>
						<div class="cont_box">
							<div>
								<p>미국 국무부 주관으로 "10대 청소년 교류계획" 및 세계문화교류의 취지로 만들어진 프로그램 입니다.</p>
								<p>세계 여러나라의 15세-18세 학생들이 연간 약 2만명 이상이 참여하고 있으며 한학기 또는 1년 동안 <br>미국 공립학교에서 미국 학생들과 동일하게 정규수업은 물론 다양한 방과후 활동과 스포츠, 이벤트에 참여하여 <br>미국의 리얼 문화와 언어를 경험하는 것입니다. 선별된  자원봉사자 가정에서 생활하며 비영리 재단소속의 <br>지역관리자들이 참가 학생이 학교와 홈스테이 가정에서 안전하고 건강하게 생활할 수 있도록 써포트 합니다.</p>
							</div>
						</div>

						<div class="cont_title">교환학생 프로그램 신청자격</div>
						<table class="cont_table">
							<colgroup>
								<col style="width:32%">
								<col>
							</colgroup>
							<tbody>
								<tr>
									<th>나이</th>
									<td>만 15-18.5세 (중3-고3)</td>
								</tr>
								<tr>
									<th>성적</th>
									<td>최근 2-3년 성적 "C"이상</td>
								</tr>
								<tr>
									<th>영어</th>
									<td>ELTIS 222점이상</td>
								</tr>
								<tr>
									<th>학업기간</th>
									<td>1학기 혹은 2학기</td>
								</tr>
								<tr>
									<th>비자</th>
									<td>문화교류비자 J1</td>
								</tr>
							</tbody>
						</table>

					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->

				<!-- cont_sect -->
				<section class="cont_sect sub2_7">
					<div class="inner">

						<div class="cont_title">교환학생 프로그램 장점</div>
						<div class="cont_image"><img src="/static/img/early_abroad/usa/sub2_7_img1@2x.png" alt=""></div>

						<div class="cont_title">공립교환 VS 일반사립</div>

						<table class="cont_table">
							<colgroup>
								<col style="width:33.33%">
								<col style="width:33.33%">
								<col>
							</colgroup>
							<thead>
								<tr>
									<th>구분</th>
									<th>J1공립교환</th>
									<th>F1일반사립</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>비자종류</td>
									<td>J1 문화교류 비자</td>
									<td>F1 학생비자</td>
								</tr>
								<tr>
									<td>지원나이</td>
									<td>만 15세-18.5세</td>
									<td>만 13세 - 18.5세</td>
								</tr>
								<tr>
									<td>참가학교</td>
									<td>미국 공립학교</td>
									<td>미국 사립학교</td>
								</tr>
								<tr>
									<td>학업기간</td>
									<td>1학기 또는 1년</td>
									<td>최소 1학기 - 제한 없음</td>
								</tr>
								<tr>
									<td>지역</td>
									<td>선택 불가능</td>
									<td>선택 가능</td>
								</tr>
								<tr>
									<td>숙소</td>
									<td>자원봉사자 호스트</td>
									<td>일반 호스트 혹은 기숙사</td>
								</tr>
								<tr>
									<td>참가비</td>
									<td>1만불대</td>
									<td>학교별로 상이</td>
								</tr>
								<tr>
									<td>졸업여부</td>
									<td>졸업 불가</td>
									<td>졸업 가능</td>
								</tr>
							</tbody>
						</table>

					</div><!-- // inner -->
				</section>
				<!-- // cont_sect -->
			<?php
				include($_SERVER['DOCUMENT_ROOT'].'/application/views/layout/quick_nav.php');
			?>
			</div>
			<!-- // content -->
		</div>
		<!-- // container -->
		<?php
		include($_SERVER['DOCUMENT_ROOT'].'/application/views/layout/footer.php');
		?>
		<div class="dim"></div>
		<?php
		include($_SERVER['DOCUMENT_ROOT'].'/application/views/layout/consult_popup.php');
		?>
	</div>
	<!-- // wrap-->
</body>
</html>


